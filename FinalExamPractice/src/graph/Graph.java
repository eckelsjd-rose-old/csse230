...
public abstract class Graph<T> {
	 ...
	 * @return
	 */
	public int numDirectedTriangles() { // Can also remove "abstract" modifier and implement here
		int count = 0;
		for (T vertex : this.keySet()) {
			Iterator<T> iter = this.successorIterator(vertex);
			while (iter.hasNext()) {
				T succ = iter.next();
				Iterator<T> iter2 = this.successorIterator(succ);
				while (iter2.hasNext()) {
					T succ2 = iter2.next();
					Iterator<T> iter3 = this.successorIterator(succ2);
					while (iter3.hasNext()) {
						T succ3 = iter3.next();
						if (succ3.equals(vertex)) { count++; }
					}
				}
			}
		}
